...
export const GoalSchema = z.discriminatedUnion('type', [
  z.object({ id: z.string(), type: z.literal('score'), target: z.number(), required: z.boolean().default(false), reward: z.string().optional(), label: z.string().optional() }),
  z.object({ id: z.string(), type: z.literal('gromLocal'), grom: z.number(), local: z.number(), required: z.boolean().default(false), reward: z.string().optional(), label: z.string().optional() }),
  z.object({ id: z.string(), type: z.literal('sectionScore'), section: z.enum(['air', 'face', 'tube']), target: z.number(), required: z.boolean().default(false), reward: z.string().optional(), label: z.string().optional() }),
  z.object({ id: z.string(), type: z.literal('noWipeout'), seconds: z.number(), score: z.number(), required: z.boolean().default(false), reward: z.string().optional(), label: z.string().optional() }),
  z.object({ id: z.string(), type: z.literal('specialTime'), seconds: z.number(), required: z.boolean().default(false), reward: z.string().optional(), label: z.string().optional() }),
  z.object({ id: z.string(), type: z.literal('rotation'), degrees: z.union([z.literal(360), z.literal(540), z.literal(720)]), count: z.number().int(), required: z.boolean().default(false), reward: z.string().optional(), label: z.string().optional() }),
...
export class GoalTracker {
    ...
    for (const p of this.progress) {
      ...
      switch (g.type) {
        ...
        case 'noWipeout':
          ...
          p.detail = `${run.rideSeconds.toFixed(0)}s · ${run.score.toLocaleString('en-US')}/${g.score.toLocaleString('en-US')}`;
          break;
        case 'specialTime':
  ...
  hudLines(): string[] {
    return this.progress.map((p) => {
      ...
      else prog = `${p.goal.type === 'specialTime' || p.goal.type === 'tubeTime' ? p.current.toFixed(1) : Math.round(p.current)} / ${p.target}`;
