...
export class RideScene implements GameScene {
  ...
  init(ctx: SceneContext): void {
    ...
    window.addEventListener('lineup:input', (e) => {
      const d = (e as CustomEvent).detail as Partial<RiderInput> | null;
      this.headlessInput = d ? { ...cloneInput(NEUTRAL_INPUT), ...d } : null;
    });
  ...
  debugState(): Record<string, unknown> {
    return { t: this.time, curlU: this.wave.curlU, rider: this.rider.snapshot(), sections: this.wave.sections.length };
  }
}
