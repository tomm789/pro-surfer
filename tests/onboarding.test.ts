...
function playLesson(id: string, seed: number, policy: Policy = 'cruise') {
  ...
  while (!run.ended && t < level.seconds + 5) {
    ...
    if (rider.state === 'prone') {
      ...
      set(0, -1, 0, -1); // land with the legs loaded
    } else if (rider.state === 'face') {
      if (policy === 'airs' && rider.speed > 9) {
  ...
  return { done, airs, topSpeed, requiredDone: goals.requiredDone, lines: goals.hudLines() };
}
